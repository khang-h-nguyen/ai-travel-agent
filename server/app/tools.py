...
def validate_destination_tool(destination: str) -> dict:
    """
    Check if a destination is in our database of Canadian cities.

    Use this tool first to validate the destination exists.
    """
    is_valid, dest_info = validate_destination(destination)

    if not is_valid:
        return {"valid": False, "error": dest_info}
...
async def get_weather_tool(city: str) -> dict:
    """
    Get current weather conditions for a Canadian city.

    Use this when planning a trip soon (not months ahead).
    """
...
def get_activities_tool(
    destination: str, interests: str, weather_description: str = "unknown"
) -> dict:
    """
...
def calculate_budget_tool(daily_rate: int, num_days: int) -> dict:
    ...
    Simple calculation: daily_rate × num_days.
    """
